 ...
 */
@Service
public class NoticeRecordServiceImpl extends ServiceImpl<NoticeRecordMapper, NoticeRecord> implements INoticeRecordService {
    ...
    public Result<?> audit(String id, String userName, Integer isPass, String remark) {
        NoticeRecord record = getById(id);
        if (record == null) {
        ...
        if (result.isSuccess()) {
            ...
            if (workFlowResult.isSuccess()) {
                boolean save = workFlowLogService.saveByTaskId(id, "NOTICE", isPass, stepId, remark);
