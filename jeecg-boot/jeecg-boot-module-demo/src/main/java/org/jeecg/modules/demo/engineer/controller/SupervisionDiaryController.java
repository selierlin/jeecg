...
public class SupervisionDiaryController extends JeecgController<SupervisionDiary, ISupervisionDiaryService> {
     ...
     * @return
     */
    @AutoLog(value = "监理日记/志-分页待办列表查询")
    @ApiOperation(value = "监理日记/志-分页待办列表查询", notes = "监理日记/志-分页待办列表查询")
    @GetMapping(value = "/todo")
    public Result<?> queryPageTODOList(SupervisionDiary records,
                                       @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
                                       @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize,
                                       HttpServletRequest req) {
        // 查询当前登录用户可以查看到的步骤
        ...
        List<Object> stepIds = workFlowStep.getResult();
        QueryWrapper<SupervisionDiary> queryWrapper = QueryGenerator.initQueryWrapper(records, req.getParameterMap());
        queryWrapper.in("step_id", stepIds);
        Page<SupervisionDiary> page = new Page<SupervisionDiary>(pageNo, pageSize);
        IPage<SupervisionDiary> pageList = supervisionDiaryService.page(page, queryWrapper);
        return Result.OK(pageList);
     ...
     * @return
     */
    @AutoLog(value = "监理日记/志-分页已办列表查询")
    @ApiOperation(value = "监理日记/志-分页已办列表查询", notes = "监理日记/志-分页已办列表查询")
    @GetMapping(value = "/complete")
    public Result<?> queryPageCompleteList(SupervisionDiary records,
                                           @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
                                           @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize,
                                           HttpServletRequest req) {
        // 查询当前登录用户可以查看到的步骤
